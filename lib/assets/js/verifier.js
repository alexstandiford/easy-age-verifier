...
function taseavIsAboveAge(ageToCheck){
  ...
  if(!ageToCheck && taseavData.formType != 'eav_confirm_age'){
    ...
    ageToCheck = taseavData.verification.minAge;
    taseavDebug('Age to check: ' + taseavData.verification.minAge);
  }
  if(taseavData.formType == 'eav_confirm_age'){
...
function confirmAge(){
  if(taseavIsAboveAge() == false){
    ...
    if(!taseavData.isCustomizer){
      history.back();
    }
  }
  else{
    taseavDebug('User is older than the min age of ' + taseavData.verification.minAge + '. Removing age verify overlay.');
