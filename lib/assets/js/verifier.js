  ...
  if(taseavData.isOfAge == false && taseavData.loggedIn != true){
    jQuery.post(taseavData.templatePath,taseavData).done(function(data){
      ...
      jQuery(data).ready(function(){
        taseavWrapper.find('form').submit(function(e){
          e.preventDefault();
        });
        //Disables mouse-wheel when gallery is open
      taseavWrapper.bind("mousewheel",function(){
          return false;
        });
        if(taseavData.formType == 'eav_confirm_age'){
          taseavWrapper.find('input[type=submit]').click(function(){
            jQuery("input[type=submit]",jQuery(this).parents("form")).removeAttr("selected");
            jQuery(this).attr("selected","true");
          });
        }
      taseavWrapper.submit(function(){
          taseavStoreAge(taseavWrapper);
          confirmAge();
