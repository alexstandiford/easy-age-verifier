...
Note that the verifier script will run exactly like it would run if debug mode is disabled. In other words, the same rules that determine if the verifier should pop up apply when you enter the function.